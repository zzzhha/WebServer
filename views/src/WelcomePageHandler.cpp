...
// 处理欢迎页面请求
void WelcomePageHandler::Handle(HttpRequest* request, HttpResponse& response) {
    // 构建完整的HTML文件路径
    std::string full_path = html_path_ + "/welcome.html";
    ...
    if (stat(full_path.c_str(), &file_stat) == 0 && S_ISREG(file_stat.st_mode)) {
        ...
        if (file.is_open()) {
            ...
            LOGINFO("成功读取欢迎页面: " + full_path + ", 大小: " + std::to_string(file_content.size()));
        } else {
